...
def dErrors(X, y, y_hat):
    DErrorsDx1 = [(y[i]-y_hat[i]) * X[i][0] for i in range(len(X))]
    DErrorsDx2 = [(y[i]-y_hat[i]) * X[i][1] for i in range(len(X))]
    DErrorsDb = [y[i]-y_hat[i] for i in range(len(X))]
...
def gradientDescentStep(X, y, W, b, learn_rate = 0.01):
    ...
    deltaE = dErrors(X,y,y_hat)
    # TODO: Update the weights
    W[0] += sum(deltaE[0])*learn_rate
    W[1] += sum(deltaE[1])*learn_rate
    b += sum(deltaE[2])*learn_rate
