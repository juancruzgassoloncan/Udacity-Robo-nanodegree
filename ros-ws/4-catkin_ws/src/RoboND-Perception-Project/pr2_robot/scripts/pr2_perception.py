...
def pcl_callback(pcl_msg):
    ...
    pcl_clean = statistical_filter(pcl_data, k_neighbors=10, scale_f=0.05)
    # TODO: Voxel Grid Downsampling
    pcl_filt = voxel_grid_filter(pcl_clean, l_size=0.01)
    # TODO: PassThrough Filter
    pcl_filt = passThrough_filter(
        pcl_filt, axis='z', axis_min=0.57, axis_max=1.1)
    pcl_filt = passThrough_filter(
        pcl_filt, axis='y', axis_min=-0.46, axis_max=0.46)
    ...
    cluster_idx = clustering_eu_dist(
        white_cloud, tol=0.05, minCsize=50, maxCsize=8000)
...
def pr2_mover(object_list):
    ...
    for i in range(len(object_list_param)):
        ...
        dic = make_yaml_dict(test_scene_n, arm, obj_name,
